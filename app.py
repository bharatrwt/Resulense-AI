"""
Resulense - Main Application
"""
import streamlit as st
from utils.resume_parser import ResumeParser
extract_text_from_pdf=ResumeParser().extract_text_from_pdf
extract_text_from_docx=ResumeParser().extract_text_from_docx

# Set page config at the very beginning
st.set_page_config(
    page_title="Resulense AI",
    page_icon="🚀",
    layout="wide"
)
import sqlite3
...
dotenv_path = os.path.join(os.path.dirname(__file__), "email_pass.env")
...
def get_db_connection():
    return sqlite3.connect('users.db', isolation_level=None)
...
# If table exists but `attempts` column is missing, add it
if "attempts" not in columns:
    cursor.execute("ALTER TABLE otp_store ADD COLUMN attempts INTEGER DEFAULT 0;")
    conn.commit()

# If `otp_store` doesn't exist, create it
if not columns:
    cursor.execute('''CREATE TABLE otp_store (
        email TEXT UNIQUE NOT NULL,
        otp TEXT NOT NULL,
...
def store_otp(email, otp):
    ...
    cursor.execute("REPLACE INTO otp_store (email, otp, timestamp, attempts) VALUES (?, ?, ?, 0)", (email, otp, timestamp))
    conn.commit()

def validate_otp(email, otp):
    cursor.execute("SELECT otp, timestamp, attempts FROM otp_store WHERE email = ?", (email,))
    record = cursor.fetchone()
    
    if record:
        stored_otp, timestamp, attempts = record
        if attempts >= 3:
            st.error("❌ Too many incorrect attempts! Request a new OTP.")
            return False
        if stored_otp == otp and (int(time.time()) - timestamp) <= 300:
            cursor.execute("DELETE FROM otp_store WHERE email = ?", (email,))
            conn.commit()
            return True
        else:
            cursor.execute("UPDATE otp_store SET attempts = attempts + 1 WHERE email = ?", (email,))
            conn.commit()
            return False
    return False
...
def auth_screen():
    if st.session_state.get("logged_in"):
        st.experimental_rerun()  # Redirect to main_app() if already logged in

    st.title("🔐 Resulense AI Authentication")
    st.markdown("### 🌟 Secure & Smart Authentication")

    option = st.radio("Choose an option:", ["Sign In", "Sign Up", "Forgot Password?"], horizontal=True)
    ...
    if option == "Sign Up":
        ...
        if st.button("✅ Sign In", use_container_width=True):
            cursor.execute("SELECT hashed_password, login_count FROM users WHERE email = ?", (email,))
            user = cursor.fetchone()

            if user and verify_password(user[0], password):
                st.session_state["logged_in"] = True
                st.session_state["email"] = email
                st.session_state["login_count"] = user[1] + 1
                cursor.execute("UPDATE users SET login_count = ? WHERE email = ?", (st.session_state["login_count"], email))
                conn.commit()
                st.success(f"✅ Welcome back, **{email}**! 🎉 You've logged in {st.session_state['login_count']} times.")
                st.rerun()
            else:
...
def main_app():
    if "logged_in" not in st.session_state or not st.session_state["logged_in"]:
        st.warning("⚠️ Please sign in to access the application.")
        time.sleep(2)  # Small delay for user notice
        st.rerun()  # Redirect to login screen
    
    st.title("🎉 Welcome to Resulense AI")
...
class ResumeApp:
    ...
    def render_about(self):
        ...
        image_base64=get_base64_image("assets/logo.jpg")
        apply_modern_styles()
        ...
        st.markdown("""
            <div class="hero-section">
                <h1 class="hero-title">About Resulense</h1>
        ...
        st.markdown(f"""
            <div class="profile-section">
                ...
                <h2 class="profile-name">Bharat Singh Rawat <svg height="25" viewBox="1 -2 22 22" width="25" class="r-1xvli5t r-3t4u6i r-4qtqp9 r-bnwqim r-f9ja8p r-lrvibr r-m6rgpd r-og9te1 r-yyyyoo" role="img" aria-label="Verified account" data-testid="icon-verified"><g><linearGradient gradientUnits="userSpaceOnUse" id="12-a" x1="4.411" x2="18.083" y1="2.495" y2="21.508"><stop offset="0" stop-color="#f4e72a"></stop><stop offset=".539" stop-color="#cd8105"></stop><stop offset=".68" stop-color="#cb7b00"></stop><stop offset="1" stop-color="#f4ec26"></stop><stop offset="1" stop-color="#f4e72a"></stop></linearGradient><linearGradient gradientUnits="userSpaceOnUse" id="12-b" x1="5.355" x2="16.361" y1="3.395" y2="19.133"><stop offset="0" stop-color="#f9e87f"></stop><stop offset=".406" stop-color="#e2b719"></stop><stop offset=".989" stop-color="#e2b719"></stop></linearGradient><g clip-rule="evenodd" fill-rule="evenodd"><path d="M13.324 3.848L11 1.6 8.676 3.848l-3.201-.453-.559 3.184L2.06 8.095 3.48 11l-1.42 2.904 2.856 1.516.559 3.184 3.201-.452L11 20.4l2.324-2.248 3.201.452.559-3.184 2.856-1.516L18.52 11l1.42-2.905-2.856-1.516-.559-3.184zm-7.09 7.575l3.428 3.428 5.683-6.206-1.347-1.247-4.4 4.795-2.072-2.072z" fill="url(#12-a)"></path><path d="M13.101 4.533L11 2.5 8.899 4.533l-2.895-.41-.505 2.88-2.583 1.37L4.2 11l-1.284 2.627 2.583 1.37.505 2.88 2.895-.41L11 19.5l2.101-2.033 2.895.41.505-2.88 2.583-1.37L17.8 11l1.284-2.627-2.583-1.37-.505-2.88zm-6.868 6.89l3.429 3.428 5.683-6.206-1.347-1.247-4.4 4.795-2.072-2.072z" fill="url(#12-b)"></path><path d="M6.233 11.423l3.429 3.428 5.65-6.17.038-.033-.005 1.398-5.683 6.206-3.429-3.429-.003-1.405.005.003z" fill="#d18800"></path></g></g></svg></h2>
                <p class="profile-title">AI/ML Enthusiast</p>
                <div class="social-links">
                    <a href="https://github.com/bharatrwt" class="social-link" target="_blank">
                        <i class="fab fa-github"></i>
                    </a>
                    <a href="https://www.linkedin.com/in/bharat-singh-rawat-99321b2a5/" class="social-link" target="_blank">
                        <i class="fab fa-linkedin"></i>
                    </a>
                    <a href="mailto:bharatrawat7163@gmail.com" class="social-link" target="_blank">
                        <i class="fas fa-envelope"></i>
                    </a>
                </div>
                <p class="bio-text">
                    Hello! I'm a passionate diploma student with expertise in Data Science and Machine Learning. 
                    I created Resulense (An Smart Resume AI) to revolutionize how job seekers approach their career journey. 
        ...
        st.markdown("""
            <div class="vision-section">
                ...
                <p class="vision-text">
                    "Resulense AI (An Smart-AI-Resume Analyzer) represents my vision of democratizing career advancement through technology. 
    ...
    def render_home(self):
        ...
        # Hero Section
        hero_section(
            "RESULENSE (An Smart-AI-Resume Analyzer)",
